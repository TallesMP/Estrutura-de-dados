...
queue_status remover_queue(queueTAD *queue) {
  if (queue == NULL)
...
queue_status enqueue(queueTAD queue, const elementoT elemento) {
  if (queue == NULL)
    return QUEUE_ERRO_QUEUE;
...
queue_status priority_enqueue(queueTAD queue, elementoT elemento,
                              int prioridade) {
  if (queue == NULL)
    return QUEUE_ERRO_QUEUE;
  if (elemento == NULL)
  ...
  // Inserir no início se a prioridade for maior que a do primeiro elemento
  else if (prioridade < queue->inicio->prioridade) {
 ...
 * elemento é colocado no endereço apontado pelo ponteiro "elemento". Retorna
 * o queue_status apropriado.
 */

queue_status dequeue(queueTAD queue, elementoT *elemento) {
  if (queue == NULL)
    return QUEUE_ERRO_QUEUE;
  else if (elemento == NULL)
    return QUEUE_ERRO_ARGUMENTO;
...
queue_status vazia(const queueTAD queue, bool *esta_vazia) {
  if (queue == NULL)
    return QUEUE_ERRO_QUEUE;
...
queue_status cheia(const queueTAD queue, bool *esta_cheia) {
  if (queue == NULL)
    return QUEUE_ERRO_QUEUE;
...
queue_status num_elementos(const queueTAD queue, size_t *nelem) {
  if (queue == NULL)
    return QUEUE_ERRO_QUEUE;
  else if (nelem == NULL)
    return QUEUE_ERRO_ARGUMENTO;
...
queue_status info(const queueTAD queue, bool *din, int *tamax) {
  if (queue == NULL)
    return QUEUE_ERRO_QUEUE;
  else if (din == NULL)
    return QUEUE_ERRO_ARGUMENTO;
...
#ifdef debug
queue_status ver_elemento(const queueTAD queue, const size_t posicao,
                          elementoT *elemento) {
  if (queue == NULL)
    return QUEUE_ERRO_QUEUE;
  else if (elemento == NULL)
